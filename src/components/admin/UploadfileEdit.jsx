...
function UploadfileEdit({ images, setImages, setIsLoading }) {
  ...
  const handleChange = (e) => {
    ...
    if (files) {
      ...
      let allfiles = images; //สำหรับเก็บข้อมูล Image หาก upload ผ่าน cloudinary สำเร็จ
  ...
  const handleRemove = (public_id) => {
    setIsLoading(true);
    const imagesAll = images; //สำหรับเก็บ image ทั้งหมดที่ได้เลือกไว้

    //Remove from cloudinary
    removeFile(public_id)
      .then((res) => {
        const filterImage = imagesAll.filter((item) => item.public_id !== public_id); //filter ภาพที่ถูกลบออก เพื่อแสดง preview
